...
year = int(input("Enter the year to get: "))

with open(f"essential-data-{year}.json", 'w') as file:
    data = Crf_dict_cursor(from_date=f"{year}-01-01", to_date=f"{year}-12-12", page_limit=12)['items']
    data_list = []
    for item in data:
        data_list.append( {
            "DOI": item.get('DOI', []), 
